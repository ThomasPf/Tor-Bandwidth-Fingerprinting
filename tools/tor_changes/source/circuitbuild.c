...
{
   /*
	* check if circuit pre-defined
	*/
  char* ch = get_relay_nickname(get_options()->Nickname, 3);
  if (ch) {
	return node_get_by_nickname(ch, 0);
  }
                          ...
                          int cur_len)
{
  ...
  /*
   * check for predefined choice
   */
  char* ch = get_relay_nickname(get_options()->Nickname,2);
  if (ch){
    choice = node_get_by_nickname(ch,0);
  } else {
...
{
	  char* ch = get_relay_nickname(get_options()->Nickname, 1);
	  if (ch) {
	     return node_get_by_nickname(ch, 0);
	  }
...
/**
 * predefined circuits for nodes
 */
char* get_relay_nickname(char* client, int pos){
	FILE *fp;
		   char buff[255];
    char* result = NULL;
    if (!get_options()->TestingPredefinedCircuitsFile){
        	return result;
    }
	fp = fopen(get_options()->TestingPredefinedCircuitsFile, "r");
    if (!fp){
    	return result;
    }
